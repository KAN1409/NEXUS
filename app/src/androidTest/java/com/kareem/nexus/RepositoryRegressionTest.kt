...
class RepositoryRegressionTest {
    ...
    @Test fun concurrentApprovalIsRecordedExactlyOnce() = runBlocking {
        ...
        repository.startAction(id); repository.completeAction(id); repository.rebuildUnderstanding()
    ...
    @Test fun deferredActionOnlyResurfacesAfterWindow() = runBlocking {
        ...
        repository.rebuildUnderstanding()
        val id = repository.actions().first().single().id
        repository.deferAction(id); repository.rebuildUnderstanding()
    ...
    @Test fun databaseReopenPreservesExistingRecords() = runBlocking {
        ...
        try {
            OfflineNexusRepository(persistent.nexusDao(), persistent).captureObservation(ObservationType.MANUAL, "Existing user memory", "NEXUS")
            persistent.close()
        ...
        } finally { persistent.close(); context.deleteDatabase(name) }
    }
}
